...
class FaceGraphic constructor(overlay: GraphicOverlay?, private val face: Face) : GraphicOverlay.Graphic(overlay) {
  ...
  private val x = translateX(face.boundingBox.centerX().toFloat())
  private val y = translateY(face.boundingBox.centerY().toFloat())
  // Calculate positions.
  private val left = x - scale(face.boundingBox.width() / 2.0f)
  private val top = y - scale(face.boundingBox.height() / 2.0f)
  private val right = x + scale(face.boundingBox.width() / 2.0f)
  private val bottom = y + scale(face.boundingBox.height() / 2.0f)
